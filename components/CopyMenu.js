...
function CopyButton(props) {
  return <Button {...props} hoverColor={COLORS.SECONDARY} color="rgba(255, 255, 255, 0.7)" padding="8px" />;
}
...
function CopyMenu({ isVisible, toggleVisibility, copyImage, carbonRef }) {
  ...
  const [showCopied, { loading: copied }] = useAsyncCallback(() => new Promise(res => setTimeout(res, 1000)));
  ...
  return (
    <div className="copy-menu-container">
      ...
      <Popout hidden={!isVisible} borderColor={COLORS.SECONDARY} pointerRight="24px" style={popoutStyle}>
        <div className="copy-row flex">
          ...
          <CopyEmbed title="IFrame" mapper={url => toIFrame(url, carbonRef.clientWidth, carbonRef.clientHeight)} />
