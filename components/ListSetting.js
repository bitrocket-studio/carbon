...
class ListSetting extends React.Component {
  ...
  renderListItems() {
    return this.props.items.map(item => (
      <div role="button" tabIndex={0} className="list-item" key={item.id} onClick={this.select.bind(null, item.id)}>
  ...
  render() {
    ...
    return (
      <div className="list-select-container">
        <div role="button" tabIndex={0} className={`display ${isVisible ? 'is-visible' : ''}`} onClick={this.toggle}>
