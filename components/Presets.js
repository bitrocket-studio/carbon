...
const Presets = React.memo(({ show, create, toggle, undo, presets, selected, remove, apply, applied, contentRef }) => {
    ...
    <div className="settings-presets">
      ...
      {show && (
        <div className="settings-presets-content" ref={contentRef}>
          {presets
            .filter(p => p.custom)
            .map(preset => (
              <Preset key={preset.id} remove={remove} apply={apply} preset={preset} selected={selected} />
            ))}
      ...
      {show && applied && (
        <div className="settings-presets-applied">
          ...
          <Button center flex="0" onClick={undo} color={COLORS.BLACK} hoverBackground="transparent" background="transparent">
            undo <span>&#x21A9;</span>
          </Button>
        </div>
      )}
